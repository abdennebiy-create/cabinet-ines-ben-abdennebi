...
export async function POST(req: Request) {
  try {
    ...
    if (apiKey) {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              { text: MASTER_PERSONA_PROMPT },
              { text: `Patient context: ${JSON.stringify(payload)}` }
            ]
          }],
          generationConfig: {
    ...
    if (isComplete) {
      ...
      if (track === "pediatric") {
        ...
        realities = [
          ...
          `Le confort digestif (${s7 || "ventre sensible"}) et la routine familiale bénéficieront d'un rééquilibrage doux sans conflit.`
        ];
        ...
        realities = [
          ...
          `Les tentatives passées (${s7 || "tisanes/probiotiques"}) et le niveau de stress confirment la nécessité d'un protocole ciblé sur le microbiote.`
        ];
      } else {
    ...
    return NextResponse.json({ error: 'AI Engine Request Failed' }, { status: 500, headers: corsHeaders });
  }
}
