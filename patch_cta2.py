...
pattern = r'<!-- The Patient File CTA -->.*?</div>'
replacement = r'''<!-- The Patient File CTA -->
...
if old_lang_change in content:
    ...
    # Maybe whitespace is slightly off
    content = re.sub(r'function changeLanguage\(lang\) \{\s*if \(\!translations\[lang\]\) return;\s*currentLang = lang;\s*document.documentElement.lang = lang;', r"function changeLanguage(lang) {\n      if (!translations[lang]) return;\n      currentLang = lang;\n      \n      document.documentElement.lang = lang;\n      document.documentElement.setAttribute('dir', lang === 'ar' ? 'rtl' : 'ltr');", content)
