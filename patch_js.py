...
# Find init() function to inject right before it
...
js = pattern_goal.sub(new_goal, js, count=1) # Replace the second one which is in changeCalorieGoal
...
# 5. Inject the badge in renderHUD
# Find the exact span rendering the goal
old_span = '<span>${labels.goal} <strong id="dailyLogGoalKcal">${this.state.dailyCalorieGoal || 2000}</strong> ${labels.kcalUnit}</span>'
# We have a line like this actually:
# <span>${labels.goal} <strong id="dailyLogGoalKcal">2000</strong> ${labels.kcalUnit}</span>
# Wait let me check the exact text
